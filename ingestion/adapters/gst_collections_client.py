...
class GSTCollectionsClient(BaseIngestionClient):
    source_name = "gst_collections"
    ...
    def __init__(
        self,
        *,
        resource_id: str = "gst-collections",
    ...
    def parse_records(self, payload: Any) -> list[dict[str, Any]]:
        ...
        if not isinstance(rows, list):
            raise ValueError("Unexpected GST collections payload.")
        ...
        for row in rows:
            ...
            records.append(
                {
                    "timestamp": timestamp,
                    "indicator_code": "INDIA_GST_COLLECTIONS",
                    "value": row.get("value") or row.get("gst_collections") or row.get("gross_gst_revenue"),
                    "revised": 0,
