...
class GovernmentCapexClient(BaseIngestionClient):
    source_name = "government_capex"
    ...
    def __init__(
        self,
        *,
        resource_id: str = "government-capex",
    ...
    def parse_records(self, payload: Any) -> list[dict[str, Any]]:
        ...
        if not isinstance(rows, list):
            raise ValueError("Unexpected government capex payload.")
        ...
        for row in rows:
            ...
            records.append(
                {
                    "timestamp": timestamp,
                    "indicator_code": "INDIA_GOVERNMENT_CAPEX",
                    "value": row.get("value") or row.get("capex") or row.get("capital_expenditure"),
                    "revised": 0,
