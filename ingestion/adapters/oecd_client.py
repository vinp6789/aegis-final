...
class OECDClient(BaseIngestionClient):
    source_name = "oecd"
    target_table = "macro_liquidity_indicators"
    endpoint_url = "https://sdmx.oecd.org/public/rest/v1/data"
    schema = (
        timestamp_field(),
    ...
    def parse_records(self, payload: Any) -> list[dict[str, Any]]:
        ...
        if not isinstance(rows, list):
            raise ValueError("Unexpected OECD SDMX payload.")
        ...
        for row in rows:
            ...
            records.append(
                {
                    "timestamp": timestamp,
                    "indicator_code": row.get("indicator_code") or row.get("series") or "OECD_INDICATOR",
                    "value": row.get("value"),
                    "revised": 0,
